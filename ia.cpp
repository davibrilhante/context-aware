...
double Pt;
int dmax;
// --------> CRIAR UMA VARIAVEL PARA RECEBER O ANGULO
int npontos;
int cenario;
char* arqname;
double NF;
...
//-----------> MODIFICAR AQUI O TAMANHO DO ARRAY
int M_BS;// = 12;
int N_BS;// = 12;
int M_UE;// = 4;
int N_UE;// = 4;
...
void create_scenario(std::default_random_engine gen,
	...
	/*=========================================================
		  	COLOCAR O ANGULO DE ENTRADA
		DA BASE PARA O USUARIO, CALCULADO PELO MEU CODIGO
	...
	while(Tsimulacao < limite){
		...
		if(cria==1){
			...
			/*========================================================================================
				...
				LEMBRAR DE SUBSTITUIR O ANGLE_RAND COM O A VARIAVEL GLOBAL QUE CRIEI PARA O ANGULO - angleUser!!!

			========================================================================================*/
			create_scenario(gen, los_rand, nlos_rand, uniform_rand, ray_randx, ray_randy, angleUser);//angle_rand);
...
//			ang_update = angupdate_rand (gen);
		...
		if(trace) {fprintf(tracefd,"%5d %d %d %d %f %f %f %f %f %f %f %f %f %d %d %f %f %d %d %d %f %f\n",Tsimulacao, tempoIA, tempoDADOS, interIA, BSbeam, USRbeam, NbeamBS, NbeamUSR, AFpuro, 10*log10(SNR), 10*log10(SNRmax), BeamBSdados, BeamUSRdados, contDADOS, contadorIA, totalCap, log2(SNRmax + 1), certo, ZERO, certoZERO, BeamMelhorBS, BeamMelhorUSR);}
